...
export default function Bilan() {
  ...
  const merged = [
    ...h1Entries.map((e) => ({ hyp: "H1", label: e.result.phenomenon_label, date: e.savedAt, detail: `${e.result.n_official_significant}/${e.result.official_signals.length} off. · ${e.result.n_social_significant}/${e.result.social_signals.length} soc.`, to: "/tester-h1" })),
    ...h1AggEntries.map((e) => ({ hyp: "H1 (agrégat)", label: "6 phénomènes", date: e.savedAt, detail: `${e.result.n_favorable_to_h1} favorable / ${e.result.n_against_h1} contre / ${e.result.n_neutral} neutre`, to: "/tester-h1" })),
    ...h2Entries.map((e) => ({ hyp: "H2", label: `${e.result.period_start} → ${e.result.period_end}`, date: e.savedAt, detail: e.result.real_network.trend.significant_at_0_05 ? "réel significatif" : "réel non significatif", to: "/tester-h2" })),
    ...h3Entries.map((e) => ({ hyp: "H3", label: e.result.phenomenon_label, date: e.savedAt, detail: `p_joint=${e.result.p_joint?.toFixed(3) ?? "n/a"}`, to: "/tester-h3" })),
    ...
    <div className="page page-bilan">
      ...
      {nothingYet && (
        <p className="source-note">
          ...
          <Link to="/tester-h1">Tester H1</Link>, <Link to="/tester-h2">Tester H2</Link> ou{" "}
          <Link to="/tester-h3">Tester H3</Link> pour commencer.
        </p>
      )}
