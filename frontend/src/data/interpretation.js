...
export const INTERPRETATION = {
  h1: {
    ...
    limits: [
      "Wikipédia n'est pas un canal de réaction instantanée : consulter ou éditer un article suppose souvent que l'événement soit déjà identifié comme notable. L'activité d'édition en particulier est structurellement réactive (on documente une fois qu'on sait quoi documenter) -- les vues peuvent parfois précéder une couverture officielle complète, mais restent elles aussi tirées par une actualité déjà en cours. Un résultat majoritairement défavorable à H1 sur Wikipédia peut donc refléter une limite de CE proxy précis (documentation encyclopédique, plus lente qu'une réaction sociale brute) plutôt qu'une réfutation de l'idée que des signaux sociaux rapides puissent précéder les statistiques officielles -- c'est précisément pourquoi Google Trends a été ajouté en complément (voir la page Conclusions publiées), avec le même résultat globalement négatif cependant.",
